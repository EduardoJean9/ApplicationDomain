      ...
      <div class="col-lg-12 text-center">
        ...
        <div class="well">
          ...
          <?php
            ...
            if(isset($_POST["submitBTN"]) ) {
              ...
              $query = mysqli_prepare($con, "INSERT INTO journaltemp (Account, Debit, Credit, Date) VALUES (?, ?, ?, ?)")
        ...
        <?php
          ...
          if(isset($_POST['validateBTN'])) {
            ...
            $query = mysqli_prepare($con, "INSERT INTO JournalCounter (Name) VALUES (?)")
              or die("Error: ". mysqli_error($con));
            ...
            if (empty($Account) && empty($Debit) && empty($Credit)) 
              ...
              if($Debit == $Credit) {
                $query = mysqli_prepare($con, "INSERT INTO `journal_transaction` (`Journal ID`,`Account Name`,`Debit`,`Credit`,`Date`,`Active`) SELECT $ID, `Account`,`Debit`,`Credit`,`Date`,$Active FROM `journaltemp`")
                  or die("Error. Could not insert into the table." . mysqli_error($con));
                mysqli_stmt_execute($query)
                  or die("Error. Could not insert into the table." . mysqli_error($con));
